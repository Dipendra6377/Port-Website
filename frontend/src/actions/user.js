...
export const addGithub = (title, url, image) => async (dispatch) => {
  try {
    dispatch({
      type: "ADD_GITHUB_REQUEST",
    });

    const { data } = await axios.post(
      "/api/v1/admin/github/add",
      { title, url, image },
      {
        headers: {
          "Content-Type": "application/json",
    ...
    dispatch({
      type: "ADD_GITHUB_SUCCESS",
      payload: data.message,
    });
  } catch (error) {
    dispatch({
      type: "ADD_GITHUB_FAILURE",
      payload: error.response.data.message,
    });
  }
};

export const deleteGithub = (id) => async (dispatch) => {
  try {
    dispatch({
      type: "DELETE_GITHUB_REQUEST",
    });

    const { data } = await axios.delete(`/api/v1/admin/github/${id}`);

    dispatch({
      type: "DELETE_GITHUB_SUCCESS",
      payload: data.message,
    });
  } catch (error) {
    dispatch({
      type: "DELETE_GITHUB_FAILURE",
      payload: error.response.data.message,
    });
  }
};
