...
export const addGithub = async (req, res) => {
  try {
    ...
    res.status(200).json({
      success: true,
      message: "Added To Github projects",
    });
  } catch (error) {
    return res.status(400).json({
      success: false,
...
export const deleteGithub = async (req, res) => {
  try {
    ...
    const video = user.github.find((video) => video._id == id);

    await cloudinary.v2.uploader.destroy(video.image.public_id);

    user.github = user.github.filter((video) => video._id != id);
